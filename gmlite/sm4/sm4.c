...
int SM4_set_key(const uint8_t *usrkey, int keylen, SM4_KEY *key)
{
    uint32_t *rk = key->key;
    uint32_t K[5];
    ...
    K[0] = GETDWORD(usrkey) ^ 0xA3B1BAC6;
    K[1] = GETDWORD(usrkey + 4) ^ 0x56AA3350;
    K[2] = GETDWORD(usrkey + 8) ^ 0x677D9197;
    K[3] = GETDWORD(usrkey + 12) ^ 0xB27022DC;
...
#define RF_E(X0, X1, X2, X3, X4, RK)             X4 = (X1) ^ (X2) ^ (X3) ^ (RK),X4 =(X0) ^ sbox0[ (X4) & 0xff] ^ sbox1[ (X4 >> 8) & 0xff] ^sbox2[ (X4 >> 16) & 0xff] ^ sbox3[ (X4 >> 24) & 0xff]

#define PUTDWORD(S, A)      (A)[0] = (uint8_t)(((S) >> 24) & 0xFF), (A)[1] = (uint8_t)(((S) >> 16) & 0xFF), (A)[2] = (uint8_t)(((S) >> 8) & 0xFF), (A)[3] = (uint8_t)((S) & 0xFF);

void SM4_encrypt_block(uint8_t *out, const uint8_t *in, const SM4_KEY *key)
{
...
void SM4_decrypt_block(uint8_t *out, const uint8_t *in, const SM4_KEY *key)
{
    const uint32_t *rk = key->key;
    uint32_t X[5];
...
int SM4_ecb_encrypt(uint8_t *out, int *out_len, const uint8_t *in, int in_len, const SM4_KEY *key, const int flag)
{
    int i = 0;
    ...
    if (SM4_ENC == flag) {
        while (in_len >= 16) {
            SM4_encrypt_block(out + i, in + i, key);
            i += SM4_BLOCK_SIZE;
        ...
        if (CRYPTO_pkcs7_pad(buf, in_len, SM4_BLOCK_SIZE) == GML_ERROR)
            return GML_ERROR;

        SM4_encrypt_block(out + i, buf, key);
        i += SM4_BLOCK_SIZE;
    ...
    else if (SM4_DEC == flag) {
        if(in_len % SM4_BLOCK_SIZE != 0 || in_len <= 0)
            return GML_ERROR;

        while (in_len >= SM4_BLOCK_SIZE) {
            SM4_decrypt_block(out + i, in + i, key);
            i += SM4_BLOCK_SIZE;
            in_len -= SM4_BLOCK_SIZE;
        }
        i -= SM4_BLOCK_SIZE;

        if (CRYPTO_pkcs7_unpad(&unpadded_len, out + i, SM4_BLOCK_SIZE) == GML_ERROR)
            return GML_ERROR;
...
/* if SM4_ecb failed, return 0. return 1 on success */
int SM4_ecb(uint8_t *out, int *out_len,
            const uint8_t *in, int in_len, 
            const uint8_t *usrkey, int keylen, 
            const int flag)
{
    SM4_KEY key;
    if (out == NULL || in == NULL || in_len < 0 || usrkey == NULL || keylen < 0)
        return GML_ERROR;

    if (SM4_set_key((const uint8_t *)usrkey, keylen, &key) == GML_ERROR)
        return GML_ERROR;

    if (SM4_ecb_encrypt(out, out_len, in, in_len, &key, flag) == GML_ERROR)
        return GML_ERROR;
    else
        return GML_OK;
}