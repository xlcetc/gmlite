...
# endif
    ...
    cpu_features->has_rdseed = ((cpu_info7[1] & CPUID_EBX_RDSEED) != 0x0);
    ...
    cpu_features->has_pclmul = ((cpu_info1[2] & CPUID_ECX_PCLMUL) != 0x0);
    cpu_features->has_aesni  = ((cpu_info1[2] & CPUID_ECX_AESNI) != 0x0);
    cpu_features->has_rdrand = ((cpu_info1[2] & CPUID_ECX_RDRAND) != 0x0);

    cpu_info[0] = 0;
    cpu_info[1] = cpu_info1[2];
    cpu_info[2] = cpu_info7[1]; /* mulx, adx */
    cpu_info[3] = 0;
...
int runtime_has_avx512f(void)
{
    return _cpu_features.has_avx512f;
}

int runtime_has_bmi2(void)
{
    return _cpu_features.has_bmi2;
}

int runtime_has_pclmul(void)
{
    return _cpu_features.has_pclmul;
}

int runtime_has_aesni(void)
{
    return _cpu_features.has_aesni;
}

int runtime_has_rdrand(void)
{
    return _cpu_features.has_rdrand;
}

int runtime_has_rdseed(void)
{
    return _cpu_features.has_rdseed;
}
